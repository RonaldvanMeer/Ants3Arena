...
namespace Ants3Arena.Api.Data.Repositories
{
    ...
    public class DirectionRepository : IBaseRepository<DirectionDto>
    {
        ...
        public async Task<IEnumerable<DirectionDto>> GetAllAsync(CancellationToken cancellationToken)
        {
            var entities = await _context.Directions.ToListAsync(cancellationToken);
            return _mapper.Map<IEnumerable<DirectionDto>>(entities);
        }
